  ...
  <basic-container>
    ...
    >
      <a-form :model="addUserForm">
        <a-form-item label="组别名" required>
          <a-input v-model="addUserForm.AKAname" placeholder="请输入组别名" />
        </a-form-item>
        <a-form-item label="TCP端口">
          <a-input v-model="addUserForm.tcpPort" placeholder="TCP端口" />
        </a-form-item>
        <a-form-item label="UDP端口">
          <a-input v-model="addUserForm.udpPort" placeholder="UDP端口" />
        </a-form-item>
      </a-form>
    </a-modal>
  </basic-container>
...
export default {
  ...
  setup() {
    ...
    const data = ref([
      { AKAname: '组1', tcpPort: '80,23', udpPort: '631' },
      { AKAname: '组2', tcpPort: '443,21', udpPort: '137' },
    ]);

    const columns = reactive([
      { title: '组别名', dataIndex: 'AKAname', key: 'AKAname' },
    ...
    const handleAddOrEditUser = () => {
      ...
      if (editMode.value) {
        // 编辑模式，找到并更新数据
        const userIndex = data.value.findIndex((user) => user.AKAname === addUserForm.AKAname);
        if (userIndex !== -1) {
